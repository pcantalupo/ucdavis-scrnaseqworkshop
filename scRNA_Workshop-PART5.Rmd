...
?FindNeighbors
experiment.aggregate <- Seurat::FindNeighbors(experiment.aggregate, reduction="pca", dims = use.pcs)
...
DimPlot(object = experiment.aggregate, pt.size=0.5, reduction = "pca", label = T)
...
dim(markers_all)
head(markers_all)
table(table(markers_all$gene))# unique(markers_all$gene)
