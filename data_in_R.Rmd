...
id <- separate(data.frame(id),col=1, into=c("ID", "length", "insertion_count", "deletion_count"), sep="\t", convert=TRUE)[,-1]
```
...
cov <- separate(data.frame(cov),col=1, into=c("ID", "coverage_range", "coverage", "bases"), sep="\t", convert=TRUE)[,-1]
```
...
```{r}
gccov <- grep("^GCD",data, value=TRUE)
gccov <- separate(data.frame(gccov), col=1, into=c("ID", "coverage_range", "coverage", "bases", "foo", "bar"), sep="\t", convert=TRUE)[,-1]
```
...
```{r}
new_is %>% distinct(inward)   # 652 distinct inward values
```
...
g <- g +
    labs( x = "insert size", y = "all pairs", title ="Mapped insert sizes", subtitle = "All Pairs", caption = "all pairs insert size")
g <- g + coord_cartesian(xlim=c(0,500), ylim=c(0,60000))
g <- g + theme_light()
plot(g)
```
...
head(actgm); tail(actgm)
```

now head the new actgm object. What did melt do?

```{r plot_base_comp}
# Don't need the 'by=variable' option
ic <- ggplot(actgm, aes(as.numeric(cycle), as.numeric(value), colour=variable))
