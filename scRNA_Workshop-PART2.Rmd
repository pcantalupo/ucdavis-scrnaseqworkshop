...
```{r}
load(file="pre_sample_corrected.RData")     # should contain the variable genes
length(VariableFeatures(experiment.aggregate)) # n = 2000
```
...
```{r}
myfiltered <- FilterGenes(object = experiment.aggregate, min.value = 5, min.cells = 50)
myfiltered  # wow only 9 genes are present in at least 50 cells with min counts of 5
```
...
mvp <- FindVariableFeatures(
  object = experiment.aggregate,
  selection.method = "mean.var.plot")
length(VariableFeatures(mvp))   # n = 670
mvp.top10 <- head(VariableFeatures(mvp), 10)
mvp.top10
p.mvp<- VariableFeaturePlot(mvp)
p.mvp <- LabelPoints(plot = p.mvp, points = mvp.top10, repel = TRUE)
p.mvp

intersect(top10, mvp.top10)   # only two of top10 are common between VST and MeanVarPlot
length(intersect(VariableFeatures(mvp), VariableFeatures(experiment.aggregate)))  # 347 HVG in common


disp <- FindVariableFeatures(
  object = experiment.aggregate,
  selection.method = "dispersion")
length(VariableFeatures(disp))   # n = 2000
disp.top10 <- head(VariableFeatures(disp), 10)
disp.top10
p.disp <- VariableFeaturePlot(disp)
p.disp <- LabelPoints(plot = p.disp, points = disp.top10, repel = TRUE)
p.disp

intersect(top10, disp.top10)   # only two of top10 are common between VST and Disp
length(intersect(VariableFeatures(disp), VariableFeatures(experiment.aggregate)))  # 620 HVG in common

CombinePlots(plots=list(vfp1, p.disp))
